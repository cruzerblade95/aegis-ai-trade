...
/**
 * User-selected markets for the read-only learning workspace.
 */
export const userWatchlistItems = sqliteTable(
  ...
  (table) => [
    uniqueIndex("user_watchlist_user_market_unique").on(
      table.userId,
      table.marketSymbol,
    ),
    index("user_watchlist_user_created_idx").on(
      table.userId,
      table.createdAt,
    ),
  ],
...
/**
 * Educational price thresholds only.
 *
 * These records do not place orders, contact a broker, or move funds.
 */
export const educationalPriceAlerts = sqliteTable(
  "educational_price_alerts",
  {
    id: text("id").primaryKey(),
  ...
  (table) => [
    index("educational_alerts_user_enabled_idx").on(
      table.userId,
      table.isEnabled,
    ),
    index("educational_alerts_market_idx").on(table.marketSymbol),
  ],
);
